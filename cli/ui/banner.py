...
never compete visually with the first message of the conversation:
    ...
    ● polyrob v0.12.0 · gemini-2.5-flash (gemini)
...
def _session_info_str(
    *,
    ...
    # A fresh install's instance id IS the framework name ("polyrob") — showing
    # "polyrob · instance polyrob" is noise; the instance part earns its place
    # only when an operator named the bot.
    if instance_id and instance_id != framework:
...
def banner_panel(
    *,
    ...
    (``● polyrob v0.12.0 · model (provider)``) and one dim context line (session,
