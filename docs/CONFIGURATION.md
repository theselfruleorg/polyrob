...
| `GOAL_BLOCKER_ESCALATION` | OFF | §7.2: when a goal trips the circuit breaker (`status='blocked'`) surface a concrete ask to the owner over the cron/delivery telegram rail instead of dying silently. Fail-open. | `agents/task/constants.py::AutonomyConfig.goal_blocker_escalation`; `agents/task/goals/escalation.py` |
