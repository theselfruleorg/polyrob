...
| `LLM_PROVIDER_REGISTRY` | ON | 024 P0 kill-switch: derive the provider lists (profiles, PROVIDER_CONFIG, schema routing, model map, fallback hierarchy, key detection) from the declarative `ProviderSpec` registry, and load user-declared providers. OFF = legacy literal lists, `providers.yaml` ignored — byte-identical pre-024 behavior. Remove after one release of bake. | `modules/llm/provider_spec.py::provider_registry_enabled` |
...
| `OBJECTIVE_GOAL_BUDGET` | `25` | Max LIVE (non-cancelled) goals one objective may have attached. Past the cap `GoalBoard.create` REFUSES a new child with a message telling the agent to finish/cancel open goals or raise an ask instead — an objective is allowed to be standing, not infinite. An objective's own `payload.goal_budget` overrides this; `0` disables the cap. The planner prompt shows each objective's `goal budget: <live>/<cap>` so it converges before hitting the wall. | `agents/task/goals/board.py::GoalBoard._objective_budget` |
| `PUBLISH_ENABLED` | OFF (not safe-local) | Register the `publish` tool (publish/publish_list/unpublish — the ship rail: copy workspace files to a stable public URL). The FIRST publish of a NEW slug needs a real approving provider (same resolution as `hf_deploy`, including the `auto_notify`→`owner_queue` remap so an unattended run can never first-publish a new public address); an already-approved slug then updates unattended. Additionally refused for a leaf/sub-agent, a forged self-wake/delegation-result turn, and a non-owner tenant. Sources are confined to the session workspace and credential files are refused. NOT in the `POLYROB_LOCAL` safe group. Never in default tool_ids; in `DELEGATE_BLOCKED_TOOLS` and the correspondent-gate high-impact set. | `tools/publish/__init__.py::publish_enabled` |
...
| `EMAIL_PROVIDER` | `auto` | Which transport backs the agent's email: `smtp` \| `agentmail`. Explicit value wins; `auto` resolves to `agentmail` iff `AGENTMAIL_API_KEY` is set (the one-env-var default-inbox path), else `smtp` (legacy, byte-identical). Unknown values fall back to `smtp`. | `core/config_policy/policy.py::email_provider` |
...
| `DEFI_TIERED_SPEND_LANE` | OFF | Proposal 023 §5.3 D3 — the explicit owner decision the T4 note reserved. `defi_trade`'s four verbs sit on `PAYMENT_APPROVAL_TOOLS`' SPEND lane, which queues them through `owner_queue` in **every** mode; unattended, that tap never comes and the trade times out. ON: a live spend whose declared `max_spend_usd` is within `DEFI_AUTONOMOUS_MAX_USD` executes without pre-approval and fires a post-execution owner notification + `payment_auto_approved` audit event (act-and-report); above the ceiling the owner queue is unchanged. The declared ceiling is a sound tiering key because `tx_guard` holds the caller to it — a call that under-declares to reach this lane is refused by the simulation. Nothing else is relaxed: simulation + delta assertion, PolicyGate, the rolling daily cap, correspondent taint, the forged-turn refusal and the halt switch all still apply, and `tx_guard` independently re-refuses over-ceiling amounts. Scoped to the tx_guard-simulated EVM verbs — the hyperliquid/polymarket order verbs and `x402_pay` never tier. **Independent of this flag:** a `dry_run=True` call is always exempt, because `_run_guarded` returns before `sign_and_send`, so gating a simulation bought no safety and cost the agent its ability to quote. | `core/config_policy/spend_lane.py::defi_spend_exemption`; wiring in `tools/controller/service.py`; `tools/controller/approval.py::make_approval_hook(exempt_fn=...)` |
...
| `COMPACTION_PROMPT_GUARD` | **ON** | Anti-injection framing on the compaction summarizer prompt + rebuilt summary (T1.3). | `core/config_policy/policy.py::compaction_prompt_guard` |
