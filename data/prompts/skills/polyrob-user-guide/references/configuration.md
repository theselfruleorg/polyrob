...
- GOAL_BLOCKED_PROVIDER_RETRY_MIN — default: 30 (minutes) — T2.1 typed block kinds: a `blocked` goal whose `payload.block_kind == "provider_outage"` (stamped by `dispatcher._fail_…
...
- CODE_EXEC_SSH_HOST — default: unset (required for CODE_EXEC_BACKEND=ssh) — Remote host the `ssh` code-exec backend (T2.7) runs `run_code` on over the system `ssh` binary. `setup()` fails loudly…
