...
derivations of it (with legacy literals kept only as the ``LLM_PROVIDER_REGISTRY``
kill-switch path). This ratchet keeps the seam tax from regrowing: any NEW
...
- the registry itself + the kill-switch literal sites (remove with the switch);
...
ALLOWLISTED_PROVIDER_AWARE_FILES = frozenset({
    # seam files: derivations + kill-switch literals (shrink when the switch goes)
    "api/openai_compat/model_map.py",
    "cli/config_store.py",
    "core/config.py",
    ...
    "agents/task/test_aux_chain.py",      # dev harness script colocated with source
    "agents/task/utils.py",               # per-provider message shaping quirks
    "agents/task/agent/core/model_introspection.py",  # provider display names
    "modules/llm/cache_hints.py",         # per-provider cache strategy table
