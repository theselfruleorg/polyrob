...
def test_send_message_passes_the_publish_context():
    """send_message must hand the mirror its session context, or a workspace
    path in its body reaches the owner as an unopenable server path."""
    src = _source()
    call = re.search(r"build_discrete_publish\((.*?)\n\t*\)", src, re.S)
    assert call, "build_discrete_publish call not found — did the emit path move?"
    # `_publish_context(self…)` — 044 T20 added a second argument
    # (`reply_to=params.reply_to`); what this pins is that the context is
    # PASSED, not its exact argument list.
    assert "_publish_context(self" in call.group(1), (
        "send_message no longer receives _publish_context; a named workspace "
        "file would be pasted as /var/lib/... instead of attached or linked")


def test_done_does_not_publish():
    """done is the internal completion record, not speech (2026-09-17). Any
    mirror here — latched or not — restores a second bubble the moment a turn
    speaks through a verb the latch does not know about (the `message` tool)."""
    src = _source()
    done_at = src.index("async def done(params: DoneAction")
    done_end = src.index("elif self.output_model is not None", done_at)
    done_code = [ln for ln in src[done_at:done_end].splitlines()
                 if not ln.lstrip().startswith("#")]
    for name in ("build_completion_publish", "build_discrete_publish", "_publish_context"):
        assert not any(name in ln for ln in done_code), f"done() calls {name}"
