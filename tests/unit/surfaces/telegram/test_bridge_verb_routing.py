...
def test_a_non_owner_is_refused():
    ...
    assert "Only the owner" in owner_ops.bridge_reply(None, "/tmp", ["solana", "base", "1"])


@pytest.mark.parametrize("args", [[], ["solana"], ["solana", "base"]])
def test_incomplete_args_show_usage_not_a_traceback(args):
    ...
    from surfaces.telegram import owner_ops
    out = owner_ops.bridge_reply("rob", "/tmp", args)
    assert "Usage: /bridge" in out
    assert "pending" in out.lower()


@pytest.mark.parametrize("bad", ["abc", "-1", "0"])
def test_a_bad_amount_is_refused_before_any_quote(bad):
    from surfaces.telegram import owner_ops
    out = owner_ops.bridge_reply("rob", "/tmp", ["solana", "robinhood", bad])
    assert "positive number" in out


def test_the_bare_form_is_a_QUOTE_not_an_execution(monkeypatch):
    ...
    from surfaces.telegram import owner_ops
    out = owner_ops.bridge_reply("rob", "/tmp", ["solana", "robinhood", "0.9"])
    assert seen["dry_run"] is True
    assert "Add `go` to execute" in out


def test_go_executes(monkeypatch):
    seen = {}
    ...
    monkeypatch.setattr("tools.defi.bridge_verb.perform_bridge", _fake)
    from surfaces.telegram import owner_ops
    owner_ops.bridge_reply("rob", "/tmp", ["solana", "robinhood", "0.9", "go"])
    assert seen["dry_run"] is False


def test_an_empty_report_never_reads_as_nothing_happened(monkeypatch):
    async def _fake(tool, params, ctx):
        from types import SimpleNamespace
