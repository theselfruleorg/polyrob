...
- ``apply_update`` takes exactly ONE (full) snapshot and restores it once;
...
# ---------------------------------------------------------------------------
# U9: one snapshot per apply — the engine restores it on any failed step
# ---------------------------------------------------------------------------
...
def test_apply_update_restores_the_one_snapshot_when_migrate_fails(tmp_path):
    home = tmp_path / "home"
    db = home / "memory.db"
    _seed(db, 10)
    ctx = UpdateContext(data_home=home, snapshots_root=tmp_path / "snaps", db_paths=[db])

    def bad_migrate():
        _seed(db, 5)  # now 15
        raise RuntimeError("boom")

    runners = UpdateRunners(install=lambda: None, migrate=bad_migrate,
                            verify=lambda: None, rollback_code=lambda: None)
    res = apply_update(ctx=ctx, runners=runners, from_version="0.5.0", to_version="0.5.1")
    assert res.ok is False and res.failed_step == "migrate" and res.rolled_back is True
    assert _count(db) == 10                      # restored from the one snapshot
