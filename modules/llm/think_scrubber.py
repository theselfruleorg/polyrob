...
UP-07: ported essentially verbatim from Reference (`agent/think_scrubber.py`,
the reference agent) — pure, zero-dependency. POLYROB wires it at the
...
__all__ = ["StreamingThinkScrubber", "scrub_think_blocks"]


def scrub_think_blocks(content: str) -> str:
    """Scrub reasoning blocks from a COMPLETE assistant string (non-streaming).

    Equivalent to feeding the whole string as one delta then flushing. Safe no-op when
    there's no '<' (the overwhelming-majority fast path), so a non-reasoning completion
    is never touched. Used by the adapter content seam where the whole completion arrives
    at once (UP-07).
    """
...
class StreamingThinkScrubber:
    ...
    def _strip_orphan_close_tags(cls, text: str) -> str:
        """Remove *boundary* orphan close tags from *text*.

        An orphan close tag has no matching open in the current scrubber
        state. It is stripped (with trailing whitespace) ONLY when it sits
        at a block boundary — the start of the text or after a newline with
        nothing but whitespace before it on that line. A close tag that
        appears mid-prose (e.g. the assistant legitimately writing
        ``"look for </think> in logs"`` or generating code/docs about
        reasoning tags) is genuine output and is preserved verbatim.

        Before boundary-gating, this stripped EVERY close tag unconditionally,
        silently deleting legitimate prose that merely mentioned one.
        """
        if "</" not in text:
            return text
            ...
            if text_lower[i:i + 2] == "</":
                for tag in cls._CLOSE_TAGS:
                    ...
                    if text_lower[i:i + tag_len] != tag_lower:
                        continue
                    # Boundary gate: only an orphan close at the start of a
                    # line (preceding text on this line is whitespace-only) is
                    # treated as noise; mid-prose mentions are kept.
                    emitted = "".join(out)
                    last_nl = emitted.rfind("\n")
                    line_prefix = emitted[last_nl + 1:] if last_nl != -1 else emitted
                    if line_prefix.strip() != "":
                        break  # mid-prose — keep the tag as literal text
